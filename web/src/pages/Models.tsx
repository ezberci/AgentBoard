...
export function Models() {
  ...
  return (
    <div className="p-6">
      <h2 className="mb-4 text-xl font-bold text-zinc-100">Models</h2>
      ...
      {health && health.length > 0 && (
        <div className="mb-4 flex flex-wrap gap-2">
          {health.map((h) => (
            <span
              key={h.id}
              className={`rounded px-2 py-0.5 text-xs font-medium ${h.env_present ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}
            >
      ...
      <form onSubmit={handleSubmit} className="mb-6 flex flex-wrap gap-2">
        ...
        {editingId != null && (
          <button
            ...
            className="rounded-md border border-border px-4 py-2 text-sm text-zinc-100 hover:bg-surface-raised"
          >
            Cancel
          </button>
        )}
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col gap-2">
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="rounded px-2 py-1 text-xs text-zinc-100 hover:bg-surface-raised"
                >
                  Edit
                </button>
                <button
          ...
          {(!models || models.length === 0) && (
            <div className="text-sm text-muted-fg">No models registered.</div>
          )}
        </div>
      )}
    </div>
